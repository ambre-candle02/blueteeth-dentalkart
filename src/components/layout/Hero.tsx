...
export function Hero() {
    ...
    return (
        <div ref={ref} className="relative min-h-[65vh] md:min-h-[85vh] w-full bg-slate-900 flex flex-col justify-center pt-12 md:pt-16 pb-12 overflow-hidden isolating">
            ...
            {/* Overlay - Optimized blur and blend */}
            <div className="absolute inset-0 bg-gradient-to-b from-slate-900/40 via-slate-900/60 to-slate-900/80 z-10 pointer-events-none" />
            <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full max-w-3xl h-[300px] bg-brand-primary/20 blur-[100px] rounded-full pointer-events-none z-10 opacity-60" />
            ...
            <div className="relative z-20 container mx-auto px-8 sm:px-12 lg:px-16 xl:px-20 max-w-[1600px] w-full flex flex-col items-center text-center -mt-12 md:-mt-16">
                ...
                {/* Badge - Optimized transition */}
                <motion.div
                    initial={{ opacity: 0, y: 10 }}
                    animate={{ opacity: 1, y: 0 }}
                    transition={{ duration: 0.4 }}
                >
                    <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-slate-800/40 backdrop-blur-sm border border-white/10 shadow-sm text-white text-[11px] md:text-sm font-bold uppercase tracking-[0.2em] mb-8">
                ...
                >
                    <Link
                        href="/shop"
                        className="group flex items-center justify-center gap-2 bg-brand-primary/90 hover:bg-brand-primary text-white text-[15px] font-semibold px-8 py-4 rounded-full transition-all active:scale-95 shadow-xl hover:shadow-2xl hover:-translate-y-1 w-full sm:w-auto backdrop-blur-sm border border-brand-primary/50"
                    >
                        Browse Catalog <ArrowRight size={18} className="group-hover:translate-x-1 transition-transform" />
                    </Link>
                    <Link
                        href="/new-clinic-setup"
                        className="flex items-center justify-center gap-2 bg-white/10 hover:bg-white/20 text-white text-[15px] font-semibold px-8 py-4 rounded-full border border-white/20 hover:border-white/40 transition-all active:scale-95 shadow-lg w-full sm:w-auto backdrop-blur-md"
                    >
